<form id="formdepto" class="flex items-center justify-center flex-1" method='POST' action="<?php echo base_url(); ?>Company/update/<?php echo $company->id_company ?>">
    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex flex-col my-2">
        <h1 class="text-3xl font-bold text-gray-500 text-center">Actualizar Empresa</h1><br>
        <div class="-mx-3 md:flex mb-6">
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                <label class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2" for="grid-first-name">
                    Nombres *
                </label>
                <input value="<?php echo $company->nombre_company ?>" name='txtNombre' class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-red rounded py-3 px-4 mb-3" id="grid-first-name" type="text" placeholder="Coca Cola" required>
            </div>
            <div class="md:w-1/2 px-3">
                <label class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2" for="grid-last-name">
                    Propietario *
                </label>
                <input value="<?php echo $company->propietario_company ?>" name="txtPropietario" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded py-3 px-4" id="grid-last-name" type="text" placeholder="Lemus Solares" required>
            </div>
        </div>
        ...
        <div class="-mx-3 md:flex mb-6">
            <div class="md:w-full px-3">
                ...
                <select id="region" name='txtRegion' class=" block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state" required>
                    <option selected value="<?php echo $company->id_region; ?>"><?php echo $company->nombre; ?></option>
        ...
        <div class="-mx-3 md:flex mb-6">
            <div class="md:w-full px-3">
                ...
                <select id="departamentos" name='txtDepartamento' class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state" required>
                    <option selected value="<?php echo $company->id_departamento; ?>"><?php echo $company->nombre_departamento; ?></option>
        ...
        <div class="-mx-3 md:flex mb-6">
            <div class="md:w-full px-3">
                ...
                <select id="municipios" name='txtMunicipio' class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state" required>
                    <option selected value="<?php echo $company->id_municipio; ?>"><?php echo $company->nombre_municipio; ?></option>
                </select>
            </div>
        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // $('#municipios').append('<option selected value="<?php echo $company->id_municipio; ?>"><?php echo $company->nombre_municipio; ?></option>');
